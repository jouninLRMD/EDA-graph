#!/usr/bin/env python
"""Run the Anderson-Darling / Kruskal-Wallis / Dunn / FDR statistical pipeline.

The script mirrors the original ``Feature_Analysis.py`` but is fully
vectorised and outputs tidy DataFrames.
"""
...
from edagraph.stats import anderson_darling_by_class, dunn_posthoc_fdr, kruskal_wallis
...
def parse_args():
    ...
    ap.add_argument("--top-k", type=int, default=5, help="Report top-K features by Kruskal-Wallis H.")
...
def main() -> int:
    ...
    dunn = dunn_posthoc_fdr(df, top, class_col=args.class_col)
    ...
    for feat, table in dunn.items():
        table.to_csv(out_dir / f"dunn_fdr_{feat}.csv")
