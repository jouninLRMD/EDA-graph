...
def test_preprocess_and_quantize():
    cfg = Config(fs_raw=1000, fs=8, window_sec=20)
    sig = _synthetic_eda(40.0)
    filt = preprocess_eda(sig, cfg)
    assert filt.size == int(40 * cfg.fs)
    q = quantize_signal(filt[: cfg.window_samples], cfg)
    assert q.min() >= 0 and q.max() < cfg.q_levels


def test_graph_features_schema():
    cfg = Config(fs_raw=1000, fs=8, window_sec=20)
    sig = _synthetic_eda(25.0)
...
def test_traditional_features_schema():
    cfg = Config(fs_raw=1000, fs=8, window_sec=20)
    window = _synthetic_eda(20.0, fs=cfg.fs_raw)
